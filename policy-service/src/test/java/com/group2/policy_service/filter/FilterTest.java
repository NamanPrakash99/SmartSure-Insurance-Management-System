...
public class FilterTest {
    ...
    public void testGateway_ValidSecret() throws Exception {
        ...
        when(request.getRequestURI()).thenReturn("/api/policies");
    ...
    @Test
    public void testGateway_SwaggerPath() throws Exception {
        GatewaySecurityFilter filter = new GatewaySecurityFilter();
        HttpServletRequest request = mock(HttpServletRequest.class);
        HttpServletResponse response = mock(HttpServletResponse.class);
        FilterChain chain = mock(FilterChain.class);

        when(request.getRequestURI()).thenReturn("/v3/api-docs/health");
        filter.doFilterInternal(request, response, chain);
        verify(chain, times(1)).doFilter(request, response);
    }
}
